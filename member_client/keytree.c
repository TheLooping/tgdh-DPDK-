...
int findLowestRightID(int nodeID)
{
    ...
    for (level = 0; level <= TREE_HEIGHT - findLevel(nodeID); level++)
    {
        ...
        int index;
        for (index = rightID; index >= leftID; index--)
        {
...
{
    // 创建目录
    if (mkdir(ENCRYPTION_KEY_DIR, 0777) == -1)
    {
...
{
    char filepath[64];
    snprintf(filepath, sizeof(filepath), "%s/%s", ENCRYPTION_KEY_DIR, STATUS_FILE);
    FILE *status_file = fopen(filepath, "wb");
    if (status_file == NULL)
    {
        printf("Error: Unable to create status file\n");
        exit(EXIT_FAILURE);
    }
    else
    {
        fwrite(&ks, sizeof(key_status), 1, status_file);
        fprintf_log(log_file, "max_size: %d, min_index: %d, max_index: %d, counts: %d", ks.max_size, ks.min_index, ks.max_index, ks.counts);
        fclose(status_file);
    }
}
key_status read_status()
{
    char filepath[64];
    snprintf(filepath, sizeof(filepath), "%s/%s", ENCRYPTION_KEY_DIR, STATUS_FILE);
    FILE *status_file = fopen(filepath, "rb");
    if (status_file == NULL)
    {
    ...
    key_status ks;
    fread(&ks, sizeof(key_status), 1, status_file);
    fclose(status_file);
...
int write_key()
{    
    rounds_key *rds_key = (rounds_key *)malloc(sizeof(rounds_key));
    rds_key->round = kt_ctx->rounds;
    memcpy(rds_key->key, BN_bn2hex(kt_ctx->nodes[0].blind_key), KEY_LEN);
    // 读取状态信息
    key_status ks = read_status();
    // 创建文件前，留出空间，最多存储max_size个文件
    int remain = ks.max_size - ks.counts;
    while (remain <= rds_key->round - ks.max_index)
    {

        char filepath[64];
        snprintf(filepath, sizeof(filepath), "%s/%d.bin", ENCRYPTION_KEY_DIR, ks.min_index);
        if (remove(filepath) != 0)
        {
            fprintf_log(log_file, "delete file maybe wrong: %s\n", filepath);
            // exit(EXIT_FAILURE);
        }
        ks.min_index++;
        ks.counts--;
        fprintf_log(log_file, "File deleted successfully: %s\n", filepath);
        remain++;
    }
    if (rds_key->round < ks.max_index)
    {
        fprintf_log(log_file, "Round %d maybe wrong\n", rds_key->round);
    }
    // 打开状态文件
    char filepath[64];
    snprintf(filepath, sizeof(filepath), "%s/%d.bin", ENCRYPTION_KEY_DIR, rds_key->round);
    FILE *key_file = fopen(filepath, "wb");
    if (key_file == NULL)
    {
        printf("Error: Unable to create status file\n");
        exit(EXIT_FAILURE);
    ...
    // 写入密钥到文件
    fwrite(rds_key->key, sizeof(char), KEY_LEN, key_file);

    // 更新状态信息
    ks.counts += rds_key->round - ks.max_index;
    ks.max_index = rds_key->round;

    write_status(ks);
    fclose(key_file);
    free(rds_key);

    fprintf_log(log_file, "write key to file: %d\n", kt_ctx->rounds);
    return 0;
}
...
rounds_key *read_key(int rounds)
{
    // 读取状态信息，判断是否存在该轮次的密钥
    key_status ks = read_status();
    if (rounds < ks.min_index || rounds > ks.max_index)
    {
        fprintf_log(log_file, "Error: Unable to find key for round %d\n", rounds);
        return NULL;
    }
    // 打开密钥文件
    char filepath[64];
    snprintf(filepath, sizeof(filepath), "%s/%d.bin", ENCRYPTION_KEY_DIR, rounds);
    ...
    // 读取密钥
    rounds_key *rds_key = malloc(sizeof(rounds_key));
    fread(rds_key->key, sizeof(char), KEY_LEN, key_file);
